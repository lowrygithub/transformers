...
MODEL_FOR_SEQUENCE_EMBEDDING_MAPPING = OrderedDict(
    [
        (BertConfig, BertForSequenceEmbedding),
    ]
)
...
class AutoModelForSequenceEmbedding:
    r"""
        :class:`~transformers.AutoModelForSequenceEmbedding` is a generic model class
        that will be instantiated as one of the sequence classification model classes of the library
        when created with the `AutoModelForSequenceEmbedding.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelForSequenceEmbedding is designed to be instantiated "
            "using the `AutoModelForSequenceEmbedding.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelForSequenceEmbedding.from_config(config)` methods."
        )
    ...
    def from_config(cls, config):
        ...
        Examples::
            ...
            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelForSequenceEmbedding.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_SEQUENCE_EMBEDDING_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_SEQUENCE_EMBEDDING_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Examples::
            ...
            model = AutoModelForSequenceEmbedding.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache.
            model = AutoModelForSequenceEmbedding.from_pretrained('./test/bert_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelForSequenceEmbedding.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
